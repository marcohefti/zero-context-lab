...
- 2026-02-15: Phase 7 Step 1-4: Added config merge precedence and implemented `zcl doctor` + `zcl gc` (age/size retention, pinned runs supported via `run.json` field `pinned: true`) with deterministic GC tests. Next: Phase 8 (Codex enrichment + skill pack).
...
- 2026-02-19: Capability-first isolation pass: added explicit attempt `isolationModel` (`process_runner|native_spawn`), added `suite run --session-isolation auto|process|native` with native-capability guard via `ZCL_HOST_NATIVE_SPAWN`, and updated docs/skill guidance to prefer native spawn while keeping process runners explicit.
- 2026-02-19: Reliability + triage pass: `suite run` now auto-writes canonical fail feedback on infra failure when feedback is missing (timeout/spawn/non-zero runner), trace failures normalize to typed `ZCL_E_TOOL_FAILED` instead of `UNKNOWN`, and run report JSON now exposes separate task/evidence/orchestration axes.
- 2026-02-19: Harness hardening pass: added PR/push CI and scheduled entropy guard workflows, moved docs checks into `./scripts/verify.sh`, and gated `zcl run --capture-raw` in CI/strict contexts behind explicit `ZCL_ALLOW_UNSAFE_CAPTURE=1` with integration tests.
...
- 2026-02-22: Minimal-mode + ops-friction pass: added mission-pack ingestion (`missionSource.path`) for suite-less campaigns, mission-level flow execution mode (`execution.flowMode=sequence|parallel`), enforced fresh-agent lifecycle contract (`runner.freshAgentPerAttempt=true`), added cleanup lifecycle `onFailure` with typed progress events, added built-in pair-gate trace profiles (`strict_browser_comparison`, `mcp_required`), and added operator summary artifacts (`campaign.summary.json`, `RESULTS.md`) with claimed-vs-verified mismatch rollups.
- 2026-02-22: Mission-only consistency + discoverability pass: standardized typed campaign policy errors (`ZCL_E_CAMPAIGN_PROMPT_MODE_VIOLATION`, `ZCL_E_CAMPAIGN_TOOL_DRIVER_SHIM_REQUIRED`) across lint/run/publish-check JSON surfaces, added 3-turn mission-only gating (`finalization.minResultTurn`, `--result-min-turn`) with typed early-turn failure, expanded `zcl contract --json` with `campaignSchema`, aligned docs/skill recipes for mission-only vs harness-aware workflows, and added self-contained no-context example lint checks (`scripts/no-context-examples-check.sh`) with fixture mission pack under `examples/missions/`.
