...
func Build(version string) Contract {
	return Contract{
		...
		Commands: []Command{
			...
			{
				ID:      "suite run",
				Usage:   "zcl suite run --file <suite.(yaml|yml|json)> [--run-id <runId>] [--mode discovery|ci] [--timeout-ms N] [--timeout-start attempt_start|first_tool_call] [--feedback-policy strict|auto_fail] [--campaign-id <id>] [--campaign-state <path>] [--progress-jsonl <path|->] [--blind on|off] [--blind-terms <csv>] [--session-isolation auto|process|native] [--parallel N] [--total M] [--mission-offset N] [--out-root .zcl] [--strict] [--strict-expect] [--shim <bin>] [--capture-runner-io] --json -- <runner-cmd> [args...]",
		...
		Errors: []Error{
			...
			{Code: "ZCL_E_MCP_MAX_TOOL_CALLS", Summary: "MCP proxy stopped after configured max tool calls.", Retryable: true},
