...
import (
	...
	"github.com/marcohefti/zero-context-lab/internal/contexts/runtime/app/enrich/claude"
	"github.com/marcohefti/zero-context-lab/internal/contexts/runtime/app/enrich/codex"
...
func EnrichCodexAttempt(attemptDir string, rolloutPath string) error {
	...
	if rolloutPath == "" {
		return &CliError{Code: codes.Usage, Message: "missing --rollout for codex enrichment"}
	}
...
func EnrichClaudeAttempt(attemptDir string, sessionPath string) error {
	...
	if sessionPath == "" {
		return &CliError{Code: codes.Usage, Message: "missing --rollout for claude enrichment"}
	}
	metrics, err := claude.ParseSessionJSONL(sessionPath)
	if err != nil {
		var parseErr *claude.SessionParseError
		if errors.As(err, &parseErr) {
			return &CliError{Code: codes.MissingEvidence, Message: parseErr.Error()}
		}
		return err
	}
